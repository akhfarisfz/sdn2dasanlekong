...
function AdminDetail() {
  ...
  const onUserUpdate = () => {
    ...
    http.privateHTTP.put(`${BASE_URL}/user/${params.id}/`, user, config).then((response) => {
  ...
  const onUserDelete = () => {
    message.confirmRemove(() => {
      ...
      http.privateHTTP.delete(`${BASE_URL}/user/${params.id}/`, config).then((response) => {
        message.success(response);
        navigate("/")
      }).catch((error) => {
        message.error(error)
  ...
  return (
    <div>
      ...
      <Link to="/admin/dashboard" className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded mt-4">
